...
const registerInformation = {
    cancelMessage: true,
    name: `${CMDname}T`,
    description: `Turns !${CMDname} off, and on, so people can teleport themself on top of the highest block above their heads.`,
    usage: `${CMDname}T`,
    example: [
        `${CMDname}T`
    ]
...
Server.command.register(registerInformation, (chatmsg) => {
    ...
    Server.runCommands([
        `playsound random.toast @a[name="${chatmsg.sender.nameTag}"] ~~~ 1 0.5`,
        `scoreboard players set @e[type=rot:cl] cl${CMDname} 1`,
        `tellraw @a[name="${chatmsg.sender.nameTag}"] {"rawtext":[{"text":"${Lang.CMDon}${CMDname}${Lang.CMDon2}"}]}`
    ]);
});