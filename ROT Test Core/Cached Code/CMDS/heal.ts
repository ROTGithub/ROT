...
const CMDTname: string = `heal`
const registerInformation = {
    cancelMessage: true,
    name: `${CMDname}`,
    description: `When you type ${Server.CP}${CMDname} in chat, I'll heal you.`,
    usage: `${CMDname}`,
    example: [
        `${CMDname}`
    ]
...
Server.command.register(registerInformation, (chatmsg) => {
    ...
    Server.broadcast(Lang.healFB, chatmsg.sender.name);
    Server.runCommand(`effect @a[name="${chatmsg.sender.name}"] instant_health 2 255 true`);
});
