...
const CMDTname: string = `kill`;
const registerInformation = {
    cancelMessage: true,
    name: `${CMDname}`,
    description: `When you type ${Server.CP}${CMDname} in chat, I'll kill you...`,
    usage: `${CMDname}`,
    example: [
        `${CMDname}`
    ]
...
Server.command.register(registerInformation, (chatmsg) => {
    ...
    Server.broadcast(Lang.killFB, chatmsg.sender.name);
    Server.runCommand(`effect @a[name="${chatmsg.sender.name}"] instant_damage 4 255 true`);
});
